...
const AnalyticsCharts = () => {
  ...
  useEffect(() => {
    if (bookings.length) calculateStats();
  }, [bookings, filter]);
  ...
  const filterBookings = () => {
    if (filter === "today") {
      ...
      return bookings.filter(
        (b) => new Date(b.service.bookTime).toDateString() === today
      );
    }
    if (filter === "month") {
      const now = new Date();
      return bookings.filter(
        (b) => new Date(b.service.bookTime).getMonth() === now.getMonth()
      );
    }
    return bookings;
  };
  ...
  const calculateStats = () => {
    ...
    const revenue = filtered.reduce(
      (sum, b) => (b.service.status === "Paid" ? sum + b.service.price : sum),
      0
    );
  ...
  const prepareData = () => {
    ...
    filterBookings().forEach((b) => {
      ...
      statusCounts[b.service.status] =
        (statusCounts[b.service.status] || 0) + 1;
    });

    return {
      services: Object.entries(serviceCounts).map(([name, count]) => ({
        name,
        count,
      })),
          ...
          {["all", "today", "month"].map((time) => (
            <button
              ...
              className={`btn btn-sm ${
                filter === time ? "btn-primary" : "btn-outline"
              }`}
            >
              {time === "all"
                ? "All Time"
                      ...
                      <tr key={b._id}>
                        ...
                        <td>
                          <div>
                            ...
                            <div className="text-sm">{b.user.email}</div>
                          </div>
                        </td>
